...
import lk.ijse.gdse.meowmate_backend.service.EmailService;
...
public class EmailServiceImpl implements EmailService {
    ...
    @Override
    public void sendAdoptionRequestEmail(String ownerEmail, Long petId, String messageText, String adopterEmail,String catName) {
        try {
            MimeMessage message = mailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(message, true, "UTF-8");

            helper.setFrom("matemeow36@gmail.com"); // must match configured username
...
//
//            String acceptUrl = "http://localhost:8080/api/adoptions/respond?requestId=" + requestId + "&action=ACCEPT";
//            String rejectUrl = "http://localhost:8080/api/adoptions/respond?requestId=" + requestId + "&action=REJECT";



            String acceptUrl = "http://localhost:8080/api/adoptions/respond?requestId="  + "&action=ACCEPT";
            String rejectUrl = "http://localhost:8080/api/adoptions/respond?requestId="  + "&action=REJECT";

            String body = "<div style='font-family:Arial, sans-serif; color:#333;'>"
                    + "<h2 style='color:#1E90FF;'>You have a new adoption request!</h2>"
                    + "<p>Your pet <strong>" + escapeHtml(catName) + "</strong> (ID: " + petId + ") has received an adoption request.</p>"
                    + "<p><strong>Message from adopter (" + escapeHtml(adopterEmail) + "):</strong></p>"
                    + "<div style='padding:12px; border-left:4px solid #1E90FF; background:#f8f9ff;'>"
                    + "<p>" + (messageText == null || messageText.isBlank() ? "<em>(no message)</em>" : escapeHtml(messageText)) + "</p>"
                    + "</div>"
